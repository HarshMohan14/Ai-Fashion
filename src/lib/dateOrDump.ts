...
Tone: catchy Hinglish / Indian slang, playful, premium, non-toxic. Do not insult the men.
Use clean Indian pop phrases like "clean-boy aura", "shaadi-ready polish",
"Rajma-chawal comfort fit", "thoda shana, full stylish", "green-flag dressing sense".
For title or archetype, prefer funny Indian labels like "KTM Lover", "Mumma's Boy Magnet",
"Rajma Chawal Boy", "Majnu Boy Radar", "Wannabe Boy Filter", "Clean Boy Collector",
"Shaadi Ready Prince", "Soft Launch Raja", "Gym Bro Dil", "Metro Crush",
"Jugaadu Gentleman", "Green Flag Launda", or invent a similar catchy label.
...
function buildFallbackResult(answers: DateOrDumpAnswer[], timeoutCount = 0): DateOrDumpResult {
  const signals = summarizeAnswers(answers, timeoutCount);
  const mainCategory = signals.winnerCategories[0] ?? 'clean fits';
  ...
  return {
    ...
    tags: [mainCategory, slangSpeed(speed), signals.winnerThemes[0] ?? 'clean-boy aura'].map((tag) => cleanText(tag, 'style', 28)),
    archetype: title,
    geminiUsed: false,
  };
}
