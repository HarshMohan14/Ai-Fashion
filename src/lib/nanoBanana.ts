...
Maintain the same vertical 9:16 portrait card format: one centered full-body model, head-to-toe visible, feet visible, standing on a clean white round pedestal in a seamless white photo studio with soft diffused lighting, narrow side margins, no square canvas, no landscape canvas, no excessive empty whitespace, no inset photo, no inner rectangle, no border, no frame, no screenshot-within-image.`;
